...
ayuda()
{
	...
	echo "El script necesita el parametro --logs y el directorio del archivo de logs de llamadas."
	echo "Formato: "$0" --logs `echo '"ubicacionLogs"'`"
 	echo "El script tiene por objetivo generar los siguientes informes:"
...
{	
	...
	contador=0
	echo " "
	echo "------------Promedio de las llamadas de cada dia--------------"
	echo " "
...
{	
	echo " "
	echo "------------Promedio de tiempo de las llamadas Usuario por Día--------------"
	local fin
...
{	
	...
	contar="0"
	echo " "
	echo "------------"$1" usuarios con mas llamadas en la semana--------------"
...
calcularLlamadasNoPromedio()
{
	...
	cantantidadNoProm=0
	echo " "
	echo "------------Llamadas que no superan la de tiempo por dia--------------"
	while [ $contador -lt ${#llamadas[*]} ]
	do
		promedio=0;
		cantLLamadas=0
...
cargarPromediosPorSemana()
{
	...
	echo ""
	echo "El usuario com más llamadas por debajo de la media en la semana es: `echo "${usuarioProm[$mayor]}" | cut -f 1 -d" "` (`echo "${usuarioProm[$mayor]}" | cut -f 2 -d" "` llamada/s)"
}
calcularMediaSemana()
{
		local fin
